...
void populate_art(art_tree *tree, char *keys, int *key_lens, int num_keys, value_data *val_arr_keys);
void measure_ops_perf(art_tree *tree, char *ops, int *ops_lens, op_t *ops_types, int num_ops, value_data *val_arr_ops);
...
int main(int argc, char *argv[])
{
    ...
    value_data *val_arr_keys = numa_alloc_onnode(sizeof(value_data) * num_keys, LOCAL_MASK);
    ...
    value_data *val_arr_ops = numa_alloc_onnode(sizeof(value_data) * num_ops, LOCAL_MASK);
    ...
    numa_free(val_arr_keys, sizeof(value_data) * num_keys);
    numa_free(val_arr_ops, sizeof(value_data) * num_ops);
    return 0;
}
...
void measure_ops_perf(art_tree *tree, char *ops, int *ops_lens, op_t *ops_types, int num_ops, value_data *val_arr_ops)
{
    ...
    for (int i = 0; i < num_ops; i++)
    {
        ...
        if (ops_type == OP_READ)
        {
            ...
            if (val)
            {
                value_data *v = (value_data *)val;
                uint64_t value0 = v->value0;
                uint64_t value1 = v->value1;
                uint64_t value2 = v->value2;
                uint64_t value3 = v->value3;
                total_val += value0;
                total_val += value1;
                total_val += value2;
                total_val += value3;
